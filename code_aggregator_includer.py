...
def aggregate_code(project_path, output_file, include_list):
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as outfile:
            ...
            # Generate and write the structure based on included files
            outfile.write("Project Structure (Including Selected Files):\n")
            outfile.write("==================\n")
            structure, _ = generate_project_structure(project_path, ignore_patterns)
            ...
            for file_path in include_list:
                ...
                if any(file_path.endswith(ext) for ext in code_extensions) or os.path.basename(file_path) in docker_files:
                    outfile.write(f"{'='*80}\n")
                    ...
                    try:
                        ...
                        file_ext = os.path.splitext(file_path)[1] or file_path
...
if __name__ == "__main__":
    ...
    # Step 3: Allow user to include files
    include_list = []
    while True:
        include_input = input("Enter the numbers of files to include (comma-separated, or 'done' to finish): ").strip()
        if include_input.lower() == 'done':
            break
        try:
            include_indices = [int(i) - 1 for i in include_input.split(',') if i.strip().isdigit()]
            include_list = [os.path.join(project_path, file_list[i]) for i in include_indices if 0 <= i < len(file_list)]
            print(f"Files included: {', '.join(include_list)}")
        except ValueError:
            print("Invalid input. Please enter valid numbers.")

    # Step 4: Generate the final output with only the included files
    aggregate_code(project_path, output_file, include_list)
